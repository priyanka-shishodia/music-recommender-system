'''
Usage:
    $ spark-submit --driver-memory=4g --executor-memory=24g --executor-cores=50 hyperparam_tuning.py
'''
...
def main(spark):
	...
	train_df = data_prep(spark, train_data)
	...
	for rank in ranks:
		for reg in regParams:
			# get ALS model
			als = ALS(userCol='user_id_index', itemCol='track_id_index', ratingCol='count', maxIter=maxIter, regParam=reg, rank=rank, implicitPrefs=True, coldStartStrategy='drop')
			# train ALS model
...
# 			print("RMSE on validation set with {} latent factors and regularization = {} is {}".format(rank, reg, rmse))
			...
			#get predictions
			recs = model.recommendForUserSubset(users, 500)			
			#recs = model.recommendForAllUsers(500) #returns DF with first column user ID, second column array of Row(track_id_index, "rating")
